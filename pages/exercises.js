...
const AnalysisComponent = ({ answerVal, analysis, wrongVal }) => {
  return (
    <>
      <div className="answer">
        <span style={{ color: "#3f6600" }}>
          <strong>正确答案：{resMapping[answerVal]}</strong>
        </span>
        {!wrongVal && wrongVal !== 0 ? (
            ...
            <Col>
              <Space>
                <Button type="primary" size="small" onClick={this.onSubmit}>
                  {this.props.isCheck ? "重做" : "提交"}
                </Button>
                <Button type="primary" size="small" onClick={this.backToList}>
                  返回列表
                </Button>
              </Space>
            </Col>
...
const mapStateToProps = (state) => {
  let yourResult = {};
  state.exerciseReslt.yourExerciseList &&
    state.exerciseReslt.yourExerciseList.forEach((e) => {
